...
def redundancy_check(original_data, region, church):
    # church와 name이 모두 일치하는 행을 찾음
    matched_row = original_data[
        (original_data['참가신청지역'] == region) & (original_data['소속교회/단체 (예: 한국교회 or 한국시민단체)'] == church)]
...
def fetch_form_data():
    try:
        ...
        try:
            with open('doubt_data.json', 'r', encoding = 'utf-8') as f:
                existing_doubt_data = json.load(f)
        except FileNotFoundError:
            existing_doubt_data = []

        existing_contact_numbers = {entry['contact'] for entry in existing_data}
        existing_doubt_contact_numbers = {entry['contact'] for entry in existing_doubt_data}

        # 폼 데이터 추출
        original_data = pd.read_csv('united.csv')
        data = []
        doubt_data = []
        new_data_added = False # 새로운 데이터가 추가되었는지 여부를 추적
        new_doubt_data_added = False
        entries = driver.find_elements(By.CSS_SELECTOR, 'ul._tbody.content')  # 각 항목의 CSS 셀렉터 확인
        for entry in entries:
            region = entry.find_element(By.CSS_SELECTOR, '.title:nth-child(8) a').text  # 지역
            church = entry.find_element(By.CSS_SELECTOR, '.title:nth-child(4) a').text  # 교회
            ...
            if contact not in existing_contact_numbers and contact not in existing_doubt_contact_numbers:
                if int(people) > 100 and redundancy_check(original_data, region, church):
                    doubt_data.append({
                        'contact': contact,
                        'people': people
                    })
                    new_doubt_data_added = True
                    continue
                data.append({
                    'contact': contact,
                    'people': people
        ...
        if new_doubt_data_added:
            existing_doubt_data.extend(doubt_data)

            with open('doubt_data.json', 'w', encoding='utf-8') as f:
                json.dump(existing_doubt_data, f, ensure_ascii=False, indent=4)
